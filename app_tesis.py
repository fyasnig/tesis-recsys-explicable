...
[data-testid="stError"] {
    ...
    border-radius: 10px !important;
    border-left: 3px solid #D85A30 !important;
}
...
.tt-icon {
    display: inline-block; color: rgba(138,136,128,0.4);
    font-size: 0.65rem; margin-left: 3px;
    vertical-align: super; cursor: help;
...
body.modo-defensa .stCaption { display: none !important; }
body.modo-defensa [data-testid="stCaptionContainer"] { display: none !important; }
...
# ── JS global: contador animado + micro-interacciones ────
st.markdown("""
<script>
(function() {
  // Contador animado para kpi-value
  ...
  // Observer para detectar cuando aparecen KPIs nuevos
  const observer = new MutationObserver(function(mutations) {
    mutations.forEach(function(m) {
      m.addedNodes.forEach(function(n) {
        if (n.nodeType === 1) {
          const els = n.querySelectorAll ? n.querySelectorAll('.kpi-value[data-target]') : [];
...
def kpi(value, label, tooltip="", color="#1D9E75", prefix=""):
    """KPI box premium con tooltip opcional y contador animado."""
    import re as _re
    raw = _re.sub(r"[^0-9.]", "", str(value))
    try:
        num = float(raw)
        data_target = f'data-target="{num}" data-prefix="{prefix}"'
        display = prefix
    except:
        data_target = ""
        display = str(value)
    ...
    if tooltip:
        tt_html = (
            '<span class="kpi-tooltip">'
            '<span class="tt-icon">ⓘ</span>'
            f'<span class="tt-text">{tooltip}</span>'
            '</span>'
        )
    return (
        '<div class="kpi-box">'
        f'  <div class="kpi-value" style="color:{color}" {data_target}>{display}</div>'
        f'  <div class="kpi-label">{label}{tt_html}</div>'
        '</div>'
    )
